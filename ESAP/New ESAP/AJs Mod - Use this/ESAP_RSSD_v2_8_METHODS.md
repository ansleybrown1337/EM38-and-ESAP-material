...
Version 2.8 preserves the corrected v2 statistical framework. Its changes are diagnostic and operational: it reports whether the bounded candidate/exchange search appears saturated, screens whether selected spacing is large relative to sampled proxy spatial structure, and reduces field runtime by avoiding an automatic second optimizer run unless explicitly requested.
...
For observation *i* with *p* retained standardized design scores, v2.8 calculates
...
Version 2.8 constructs a full second-order central composite design (CCD) in *p* standardized PC dimensions for *p* = 1, 2, 3, or 4. The one-PC case supports a single drone proxy such as NDVI or NDRE; the two-PC case supports joint designs from both proxies. The base design contains:
...
No arbitrary weighted sum is used. The convergence table records start type, initial and final geoMSD, swaps, sweeps, final mean and maximum target mismatch, whether the start produced a new best design, best-so-far geoMSD, starts since the last improvement, and the optimizer stop reason.

By default, `N_OPTIMIZER_STARTS` is treated as a maximum rather than a mandatory count. The optimizer runs at least `MIN_OPTIMIZER_STARTS`, then stops early if the best design survives `OPTIMIZER_NO_IMPROVEMENT_PATIENCE` additional starts without improvement. A full deterministic rerun for reproducibility can still be enabled, but it is disabled by default because it doubles field runtime without changing the recorded seed or convergence trace.

## 12. Candidate-search sufficiency diagnostics

Version 2.8 does not claim a global combinatorial optimum. Instead, it reports bounded-search evidence that helps the analyst decide whether the explored candidate space was adequate.

For every target instance, the notebook exports a candidate-pool report with pool size, minimum/median/maximum PCA target distance, final PCA tolerance, tolerance relative to the configured maximum, and whether nearest-*k* fallback was implied. This identifies targets whose statistical neighborhoods were sparse or constrained.

After coordinate exchange, the search-sufficiency report records the best geoMSD, first and last start producing the best design within tie tolerance, starts run, starts after the last best design, near-best start count, assignment-expansion attempts, pool-size summaries, targets at maximum tolerance, fallback targets, and a status:

- `plausibly_saturated_local_search` means the best design persisted through the configured no-improvement patience and no candidate-pool constraint flags were raised.
- `late_improvement_possible` means the best design appeared late enough that more starts may be useful.
- `candidate_space_constrained` means one or more target neighborhoods, tolerances, forced sites, or uniqueness constraints limited the search and should be reviewed.
- `review_optimizer_trace` means the convergence history does not support a clear automatic conclusion.

This diagnostic answers whether the implemented search appears sufficiently explored for the configured candidate pools. It is not a mathematical proof that no untested complete design could improve geoMSD.

## 13. Proxy spacing plausibility diagnostic for ordinary regression

Lesch physically separated SRS/RSSD calibration sites to improve the chance that a future ordinary regression can be fitted with approximately independent residuals. At site-selection time those residuals do not exist, so v2.8 does not optimize or test residual autocorrelation. Instead, it adds a pre-calibration spacing plausibility screen based on the spatial structure of the standardized design PC scores.

The diagnostic samples coordinate pairs directly from the supplied projected X/Y coordinates. It combines nearest-neighbor pairs from a reproducible subset with random long-distance pairs, then bins pair distances and calculates a proxy semivariance:

\[
\gamma(h) = \frac{1}{2p}\sum_{j=1}^{p}(PC_{a,j}-PC_{b,j})^2.
\]

The far-distance mean semivariance provides a reference sill-like value. The reported proxy decorrelation distance is the first interpolated distance at which binned semivariance reaches the configured fraction of that far-distance reference, 0.90 by default. Selected-site minimum nearest-neighbor spacing, median nearest-neighbor spacing, and geoMSD are compared with this proxy distance.

The resulting status may indicate that the selected minimum spacing exceeds the proxy range, that typical spacing exceeds the proxy range but a close pair remains, that spacing is near the proxy range, or that selected spacing is shorter than sampled proxy structure. This is a practical screen, not proof of residual independence. The future calibration model still requires residual diagnostics, including Moran's I when appropriate.

The diagnostic makes no regular-grid assumption. It does not infer rows or columns, reshape data, use `np.roll`, use raster FFT autocorrelation, or assume constant survey spacing. All pair distances come from the projected coordinate columns selected by the analyst.

## 14. Why this implementation does not require an N by N spatial distance matrix
...
The v2.8 proxy spacing screen also avoids full distance matrices. It uses a bounded reproducible subset, local cKDTree neighbor pairs, and a fixed number of random coordinate pairs.

## 15. Why Moran's I is excluded from site selection
...
Version 2.8 therefore does not calculate Moran's I and does not install PySAL or `esda`. Residual spatial dependence must be evaluated in the future calibration and validation workflow.

## 16. Memory scaling and large-data behavior
...
When at least two selected sensor features are available, the PCA preview defaults to two design components. The response-surface preview then displays standardized PC1 against PC2, CCD target crosses, the empirical design-radius circle, and target-tolerance rings. A one-dimensional line is shown only when one component is deliberately selected or only one sensor feature exists; the interface states this explicitly.

After a successful run, an optional final cell creates a timestamped ZIP archive. The archive contains selected and candidate tables, the candidate-pool report, target instances, optimizer convergence, PCA validation, feature skewness, search-sufficiency JSON, spacing-diagnostic JSON and CSV, configuration and metadata JSON, the run summary, preferred-location matches when applicable, a manifest, regenerated high-resolution PNG diagnostics, and a labeled Google Earth KMZ when the source projected CRS is known. The raw input file is not copied into the archive, but its name/run label is recorded.

The KMZ writer transforms selected projected coordinates to WGS84 longitude/latitude with `pyproj` and stores one placemark per selected sample. Placemark labels combine sample order and sample ID; popup descriptions retain target assignment, PCA mismatch, nearest-selected-neighbor distance, projected coordinates, and source EPSG. UTM conversions performed in the notebook supply their EPSG automatically. For data already supplied as projected easting/northing, the user must enter the correct projected EPSG in the coordinate cell. The analysis can still run when that EPSG is unknown, but KMZ generation is explicitly skipped because interpreting unlabeled projected coordinates as longitude/latitude would produce invalid locations.

## 17. Optional approximate PCA-space prefilter
...
## 18. Design diagnostics

Final spatial diagnostics include geoMSD, absolute minimum separation, mean and median nearest-selected-neighbor separation, each individual nearest-neighbor distance, candidate-search sufficiency status, and the proxy spacing plausibility screen. Model-space diagnostics include mean and maximum target mismatch, PC balance (the Euclidean norm of mean selected standardized PC scores), outlier count, and empirical design-radius coverage.
...
- The proxy spacing diagnostic assumes spatial structure in the selected sensor-PC predictors is relevant context for judging residual-independence risk, but it is not a residual covariance estimate.
...
Coordinate exchange can converge to a local optimum; multiple starts reduce but do not eliminate that risk. Candidate restrictions trade exact target matching against possible spatial separation. The search-sufficiency report describes bounded local-search saturation, not global optimality. Residual independence remains an empirical property of the future fitted model and must be checked after response collection.
...
## 21. Differences from classical ESAP software

Version 2.8 retains Lesch's SRS logic but provides transparent Python configuration, generalized multi-feature PCA inputs, empirical-radius CCD construction through four PCs, explicit balanced target replication, exact tree-based geoMSD, multiple-start coordinate exchange, large-data memory modes, reproducible metadata, bounded-search sufficiency reporting, proxy spacing plausibility diagnostics, and modern regression-design diagnostics. It does not reproduce undocumented interface defaults or binary file conventions from classical software.
...
## 22. Differences from the previous AJ notebook
...
Version 2.8 replaces those elements with standardized PC scores, multivariate chi-square radial screening, a mathematically defined multidimensional CCD, an unscaled exact geoMSD objective, candidate-constrained lexicographic optimization, unique assignment, coordinate exchange, bounded-search diagnostics, proxy spacing diagnostics, and post-calibration-only Moran guidance. Unneeded kriging, shapely, PySAL, seaborn, and pyDOE dependencies are removed. `ipywidgets` is retained only for the guided Colab interface and does not participate in the numerical method.

## 23. Planned validation against classical RSSD

Planned scientific validation should use common survey inputs and fixed response-surface specifications to compare classical ESAP and v2.8 on target mismatch, individual minimum separation distances, geoMSD, matrix rank, condition number, leverage, avePVar, search-sufficiency status, proxy spacing diagnostics, and computation time. When measured response data are available, both designs should also be compared on calibration accuracy and fitted residual spatial diagnostics. Optional approximate-prefilter runs require a separate comparison with complete-data candidate discovery.

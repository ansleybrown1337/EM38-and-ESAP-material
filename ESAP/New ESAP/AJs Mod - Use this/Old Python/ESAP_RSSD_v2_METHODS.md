...
After a successful run, an optional final cell creates a timestamped ZIP archive. The archive contains selected and candidate tables, target instances, optimizer convergence, PCA validation, feature skewness, configuration and metadata JSON, the run summary, preferred-location matches when applicable, a manifest, twelve regenerated high-resolution PNG diagnostics, and a labeled Google Earth KMZ when the source projected CRS is known. The raw input file is not copied into the archive, but its name/run label is recorded.

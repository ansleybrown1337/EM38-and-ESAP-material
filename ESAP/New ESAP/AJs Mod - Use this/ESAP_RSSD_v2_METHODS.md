...
The Colab notebook provides a sequential `ipywidgets` workflow modeled on the AJ notebook rather than a single configuration panel. Separate cells handle primary and optional secondary file upload, upload cancellation/reset, existing-location entry, dataframe preview, coordinate-system declaration and optional UTM conversion, ID selection, X/Y assignment and geographic preview, multi-select PCA features, per-feature transformations, PCA component selection, outlier and design-radius controls, sample budget, candidate settings, optimizer starts, memory mode, approximation permission, and plot colors. Upload selection immediately reports filename and size; a milestone progress indicator reports selection, parsing, validation, success, or failure. A direct `google.colab.files.upload()` fallback is included for browser/widget compatibility. Applying the final workflow cell updates the same recorded configuration object used by scripted runs; the statistical engine is not duplicated.
...
After a successful run, an optional final cell creates a timestamped ZIP archive. The archive contains selected and candidate tables, target instances, optimizer convergence, PCA validation, feature skewness, configuration and metadata JSON, the run summary, preferred-location matches when applicable, a manifest, and twelve regenerated high-resolution PNG diagnostics. The raw input file is not copied into the archive, but its name/run label is recorded.
